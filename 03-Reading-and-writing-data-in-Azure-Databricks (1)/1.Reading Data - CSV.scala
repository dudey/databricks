...
// MAGIC %python
// MAGIC # A reference to our tab-separated-file
// MAGIC csvFile = "/mnt/training/wikipedia/pageviews/pageviews_by_second.tsv"
// MAGIC 
// MAGIC tempDF = (spark.read           # The DataFrameReader
// MAGIC    .option("sep", "\t")        # Use tab delimiter (default is comma-separator)
// MAGIC    .csv(csvFile)               # Creates a DataFrame from CSV after reading in the file
// MAGIC )
...
// MAGIC %python
// MAGIC (spark.read                    # The DataFrameReader
// MAGIC    .option("sep", "\t")        # Use tab delimiter (default is comma-separator)
...
// MAGIC    .option("header", "true")       # Use first line of all files as header
// MAGIC    .option("sep", "\t")            # Use tab delimiter (default is comma-separator)
// MAGIC    .option("inferSchema", "true")  # Automatically infer data types
// MAGIC    .csv(csvFile)                   # Creates a DataFrame from CSV after reading in the file
// MAGIC    .printSchema()
// MAGIC )
...
// MAGIC   StructField("timestamp", StringType(), False),
// MAGIC   StructField("site", StringType(), False),
...
// MAGIC %python
// MAGIC (spark.read                   # The DataFrameReader
// MAGIC   .option('header', 'true')   # Ignore line #1 - it's a header
// MAGIC   .option('sep', "\t")        # Use tab delimiter (default is comma-separator)
// MAGIC   .schema(csvSchema)          # Use the specified schema
// MAGIC   .csv(csvFile)               # Creates a DataFrame from CSV after reading in the file
// MAGIC   .printSchema()
// MAGIC )
...
// MAGIC %python
// MAGIC csvDF = (spark.read
// MAGIC   .option('header', 'true')
// MAGIC   .option('sep', "\t")
// MAGIC   .schema(csvSchema)
// MAGIC   .csv(csvFile)
// MAGIC )
// MAGIC print("Partitions: " + str(csvDF.rdd.getNumPartitions()) )
// MAGIC printRecordsPerPartition(csvDF)
// MAGIC print("-"*80)
